...
<body>
    ...
    <div class="up">
        <div class="search_container">
            <form method="GET" action="{{route('sellersSearch')}}">
        ...
        <div class="title">
            <h1>المسوقين</h1>
            <a href="{{route('createSeller')}}">إضافة مسوق</a>
            <div class="icon">
    ...
    <table><tr>
        ...
        <th>حذف</th>
            ...
            @foreach ($sellers as $seller)
                <tr>
                    <td data-cell="رقم المسوق">{{$seller->id}}</td>
                    <td data-cell="اسم المسوق">{{$seller->name}}</td>
                    <td data-cell="هاتف المسوق">{{$seller->number}}</td>
                    <td data-cell="الصفقات المنتهية">{{$seller->deals()->where('done',1)->count()}}</td>
                    <td data-cell="التفاصيل"><a href="{{route('showSeller',$seller)}}" class="show">تفاصيل</a></td>
                    <td data-cell="تعديل"><a href="{{route('editSeller',$seller)}}" class="edit">تعديل</a></td>
                    <td data-cell="حذف"><form method="POST" action="{{route('deleteSeller',$seller)}}" >
                        @csrf
                        @method('delete')
